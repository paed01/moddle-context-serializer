...
ContextApi.prototype.getProcessById = function getProcessById(processId) {
  return this.elements.processes.find(({
    id
  }) => id === processId);
};
...
ContextApi.prototype.getSequenceFlowById = function getSequenceFlowById(flowId) {
  return this.elements.sequenceFlows.find(({
    id
  }) => id === flowId);
};
...
ContextApi.prototype.getExtendContext = function getExtendContext() {
  return new ExtendContext({
    scripts: this.elements.scripts,
    timers: this.elements.timers
  });
};
...
ContextApi.prototype.getScriptsByElementId = function getScriptsByElementId(elementId) {
  return this.elements.scripts.filter(({
    parent
  }) => parent.id === elementId);
};
...
ContextApi.prototype.getTimersByElementId = function getTimersByElementId(elementId) {
  return this.elements.timers.filter(({
    parent
  }) => parent.id === elementId);
};
      ...
      case 'bpmn:Collaboration':
        {
          if (element.messageFlows) {
            const {
              messageFlows: flows
            } = this._prepareElements(parent, element.messageFlows);
...
Mapper.prototype._prepareElementBehaviour = function prepareElementBehaviour(element, behaviour) {
  ...
  const preparedElement = { ...behaviour,
    ...element,
    ...(eventDefinitions ? {
      eventDefinitions
    } : undefined)
  ...
  if (properties) {
    const props = preparedElement.properties = {
      type: 'properties',
      values: []
    };
...
Mapper.prototype._prepareIoSpecificationBehaviour = function prepareIoSpecificationBehaviour({
  dataInputs,
  dataOutputs
}) {
  const result = {};
  ...
  if (dataOutputs) {
    ...
    for (const output of dataOutputs) {
      result.dataOutputs.push({ ...output,
        type: output.$type,
...
ExtendContext.prototype.addScript = function addScript(scriptName, {
  id,
  scriptFormat,
  body,
  resource,
  ...
  this.scripts.push({ ...this._prepare(scriptName, parent || this._parent),
    script: { ...(id ? {
        id
      } : undefined),
      scriptFormat,
...
ExtendContext.prototype.addTimer = function addTimer(timerName, {
  id,
  timerType,
  type,
  value,
  ...
  this.timers.push({ ...this._prepare(timerName, parent || this._parent),
    timer: { ...(id ? {
        id
      } : undefined),
      timerType,
...
ExtendContext.prototype._prepare = function prepare(name, {
  id,
  type
} = {}) {
  return {
